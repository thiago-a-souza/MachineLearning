...
A common problem in decision trees is called **overfitting**. That happens when the model works well on the training data but doesn't generalize appropriately. Several factors can cause overfitting, for example, noises in the training data can result in complex trees that don't represent the entire distribution, or when the number of examples in the training set is small and doesn't produce a significant structure. In order to solve this issue, post-pruning has been successfully applied, eliminating nodes that are not relevant - if the node is irrevelevant it is replaced with a leaf node.
